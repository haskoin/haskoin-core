...
tests =
   [  testGroup "BIP32 derivation vector 1"
        (map testVectorDataToTestCase xKeyVec1)
    , testGroup "BIP32 subkey derivation vector 2"
        (map testVectorDataToTestCase xKeyVec2)
  ...
  where testVectorDataToTestCase :: (String, XPrvKey, [ByteString]) -> Test
        testVectorDataToTestCase (path, xpk, v) = testCase ("Path " ++ path) $ runXKeyVec (path, xpk, v)
...
testParsePath = do
    ...
    return $ testCase ("Path " ++ path) $
        assertBool path (t . fromHaskoinString $ path)
...
testDerivePathE = do
    ...
    return $ testCase (toHaskoinString key ++ " Path " ++ path) $
        assertEqual path final $ 
          eitherToMaybe =<< 
            ( derivePathE <$> fromHaskoinString path <*> pure key )


-- maybe we don't want to test this
-- maybe we don't even want to export HardPath / SoftPath data types, just DerivPath
testDerivePubPath :: [Test]
testDerivePubPath = do
    (key, path, final) <- derivePubPathVectors
    return $ testCase (toHaskoinString key ++ " Path " ++ path) $
        assertEqual path final $ 
            derivePubPath <$> fromHaskoinString path <*> pure key 

-- maybe need to change the name here... not sure to what yet...
testDeriveHardPrvPath :: [Test]
testDeriveHardPrvPath = do
    (key, path, final) <- deriveHardPrvPathVectors
    return $ testCase (toHaskoinString key ++ " Path " ++ path) $
        assertEqual path final $
            deriveHardPrvPath <$> fromHaskoinString path <*> pure key


-- | todo: test also without the leading "M" (?)
derivePubPathVectors :: [(XPubKey, String, Maybe XPubKey)]
derivePubPathVectors =
    [ -- DerivPath, but not soft path
      ( xpub, "M", Nothing )
    , ( xpub, "M/8", Nothing )
...
-- either remove m/M specifier at the beginning, or remove test completely
deriveHardPrvPathVectors :: [(XPrvKey, String, Maybe XPrvKey)]
deriveHardPrvPathVectors =
    [ -- DerivPath, but not HardPath
      ( xprv, "m", Nothing )
    , ( xprv, "M", Nothing) 
    , ( xprv, "m/8'", Nothing)
    , ( xprv, "M/8'", Nothing ) 
    , ( xprv, "M/8'/30/1", Nothing ) 
    , ( xprv, "m/3/20", Nothing )
    , ( xprv, "3/20", Nothing ) -- SoftPath, but not a hard path
    , ( xprv, "8'", Just . hardSubKey' xprv $ 8 ) 
    , ( xprv, "8'/30/1", Just . foldl prvSubKey' (hardSubKey' xprv 8) $ [30,1] ) 
    , ( xprv, "kumbayah my lord", Nothing) 
    ]



derivePathVectors :: [(Bip32XKey, String, Maybe Bip32XKey)]
derivePathVectors =
    [ ( Bip32PrvK xprv, "m", Just . Bip32PrvK $ xprv )
    , ( Bip32PubK xpub, "m", Nothing )  

    , ( Bip32PrvK xprv, "M", Just . Bip32PubK $ xpub )
    , ( Bip32PubK xpub, "M", Just . Bip32PubK $ xpub )

    , ( Bip32PrvK xprv, "m/8'", Just . Bip32PrvK . flip hardSubKey' 8 $ xprv )
    , ( Bip32PubK xpub, "m/8'", Nothing )

    , ( Bip32PrvK xprv, "M/8'", Just . Bip32PubK . deriveXPubKey . flip hardSubKey' 8 $ xprv )

    , ( Bip32PrvK xprv, "m/8'/30/1", Just . Bip32PrvK . foldl prvSubKey' (hardSubKey' xprv 8) $ [30,1] )

    , ( Bip32PrvK xprv, "M/8'/30/1", Just . Bip32PubK . deriveXPubKey $ foldl prvSubKey' (hardSubKey' xprv 8) $ [30,1] ) -- foldl1 here?

    , ( Bip32PrvK xprv, "m/3/20", Just . Bip32PrvK . foldl prvSubKey' xprv $ [3,20] )
    , ( Bip32PrvK xprv, "M/3/20", Just . Bip32PubK . deriveXPubKey $ foldl prvSubKey' xprv $ [3,20] ) 
    , ( Bip32PrvK xprv, "supercalafraga", Nothing )
    ]



runXKeyVec :: (String, XPrvKey, [ByteString]) -> Assertion
runXKeyVec (_, privKey, v) = do
...
root1 :: XPrvKey
root1 =  makeXPrvKey $ fromJust $ decodeHex $ seed1
seed1 :: ByteString
seed1 = "000102030405060708090a0b0c0d0e0f"


xKeyVec1 :: [ (String, XPrvKey, [ByteString] )]
xKeyVec1 =
    [ 
      ( "m", 
        root1, 
    ...
    , ( "m/0'/1", 
        flip hardSubKey' 0 >>> flip prvSubKey' 1 $ root1, 
    ...
    , ( "m/0'/1/2'", 
        flip hardSubKey' 0 >>> flip prvSubKey' 1 >>> flip hardSubKey' 2 $ root1, 
    ...
    , ( "m/0'/1/2'/2", 
        flip hardSubKey' 0 >>> flip prvSubKey' 1 >>> flip hardSubKey' 2 >>> flip prvSubKey' 2 $ root1, 
    ...
    , ( "m/0'/1/2'/2/1000000000", 
        flip hardSubKey' 0 >>> flip prvSubKey' 1 >>> flip hardSubKey' 2 >>> flip prvSubKey' 2 >>> flip prvSubKey' 1000000000 $ root1, 
...
root2 :: XPrvKey
root2 =  makeXPrvKey $ fromJust $ decodeHex $ seed2
seed2 :: ByteString
...
prvSubKey' :: XPrvKey -> Integer -> XPrvKey
prvSubKey' xk i = prvSubKey xk (XKeySoftIndex . fromInteger $ i)

hardSubKey' :: XPrvKey -> Integer -> XPrvKey
hardSubKey' xk i = hardSubKey xk (XKeyHardIndex . fromInteger $ i)

pubSubKey' :: XPubKey -> Integer -> XPubKey
pubSubKey' xk i = pubSubKey xk (XKeySoftIndex . fromInteger $ i)


xKeyVec2 :: [ (String, XPrvKey, [ByteString] ) ]
xKeyVec2 =
    [
      ( "m", 
        root2, 
    ...
    , ( "m/0/2147483647'", 
        flip prvSubKey' 0 >>> flip hardSubKey' 2147483647 $ root2, 
    ...
    , ( "m/0/2147483647'/1", 
        flip prvSubKey' 0 >>> flip hardSubKey' 2147483647 >>> flip prvSubKey' 1 $ root2, 
    ...
    , ( "m/0/2147483647'/1/2147483646'", 
        flip prvSubKey' 0 >>> flip hardSubKey' 2147483647 >>> flip prvSubKey' 1 >>> flip hardSubKey' 2147483646 $ root2, 
    ...
    , ( "m/0/2147483647'/1/2147483646'/2", 
        flip prvSubKey' 0 >>> flip hardSubKey' 2147483647 >>> flip prvSubKey' 1 >>> flip hardSubKey' 2147483646 >>> flip prvSubKey' 2 $ root2, 
...
testCyclePrvKeys, testCyclePubKeys, testCycleHardKeys, testWord31Overflow :: Test
testCyclePrvKeys = testCase "testCyclePrvKeys" $ assertEqual "ten prv keys, starting at 2147483647 - 5"
  ( take 10 . flip prvSubKeys lastButFiveSoft  $ xprv ) $
  ( take  6 . flip prvSubKeys lastButFiveSoft  $ xprv ) 
     ++ ( take 4 . flip prvSubKeys ( XKeySoftIndex 0 ) $ xprv) 

testCyclePubKeys = testCase "testCyclePubKeys" $ assertEqual "ten pub keys, starting at 2147483647 - 5"
  ( take 10 . flip pubSubKeys lastButFiveSoft  $ xpub ) $
  ( take  6 . flip pubSubKeys lastButFiveSoft  $ xpub ) 
     ++ ( take 4 . flip pubSubKeys ( XKeySoftIndex 0 ) $ xpub) 

testCycleHardKeys = testCase "testCycleHardKeys" $ assertEqual "ten hard keys, starting at 2147483647 - 5"
  ( take 10 . flip hardSubKeys lastButFiveHard   $ xprv ) $
  ( take  6 . flip hardSubKeys lastButFiveHard   $ xprv ) 
     ++ ( take 4 . flip hardSubKeys ( XKeyHardIndex 0 ) $ xprv) 


testWord31Overflow = testCase "Word31Overflow" $ assertEqual "word31Overflow is Nothing" word31Overflow Nothing
  where  word31Overflow :: Maybe XKeyChildIndex
         word31Overflow = fromHaskoinString "2147483648"     

lastButFiveHard :: XKeyHardIndex
lastButFiveHard = ( XKeyHardIndex $ lastKeyIndex - 5 )
lastButFiveSoft :: XKeySoftIndex
lastButFiveSoft = ( XKeySoftIndex $ lastKeyIndex - 5 )

lastKeyIndex :: Integral a => a
lastKeyIndex = 2147483647
