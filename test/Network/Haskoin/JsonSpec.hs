module Network.Haskoin.JsonSpec
    ( spec
    ) where
...
spec :: Spec
spec = do
    describe "btc json tests" $ props btc
    describe "bch json tests" $ props bch
...
props net = do
    ...
    it "encodes and decodes transaction hash" $ forAll arbitraryTxHash testID
    ...
    it "encodes and decodes public key" $ forAll arbitraryPubKey (testID . snd)
