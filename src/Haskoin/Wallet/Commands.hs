...
yamlAddrList addrs acc = array 
    [ object [ (T.pack "Account") .= yamlAcc acc ]
    , object [ (T.pack "Addresses") .= (array $ map yamlAddr addrs) ]
    ]
...
cmdDumpKey opts args = whenArgs args (<= 1) $ do
    ...
    let prvKey = runAccPrvKey prv
        ...
        ms | isMSAcc acc = [object [(T.pack "MSKeys") .= array msJson]]
           | otherwise   = []
    return $ array $
        [ object [ (T.pack "Account") .= yamlAcc acc ]
...
cmdImportTx opts args = whenArgs args (== 1) $ do
    ...
    return $ object
        ...
        , (T.pack "Imported coins") .= array json
        ]
...
cmdCoins opts args = whenArgs args (<= 1) $ do
    ...
    return $ array $
        [ object [ (T.pack "Account") .= yamlAcc acc ]
        , object [ (T.pack "Coins") .= (array $ map (flip yamlCoin acc) coins) ]
        ]
...
cmdBuildTx opts args = whenArgs args (>= 2) $ do
    ...
    return $ toJSON $ bsToHex $ encode' tx
...
cmdSignTx opts args = whenArgs args (>= 2) $ do
    ...
    return $ toJSON $ show $ (bsToHex . encode') <$> sigTx
    where f [t,i,s] = do
